...
resource "aws_vpc" "main" {
  ...
  tags = merge({ "Name" = var.name }, var.vpc_tags, var.tags)
}
...
resource "aws_default_security_group" "default" {
  ...
  tags = merge({ "Name" = "default-${var.name}" }, var.tags)
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge({ "Name" = "${var.environment}-${element(var.zones, count.index)}-public" }, var.public_subnet_tags, var.tags)
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge({ "Name" = "${var.environment}-${element(var.zones, count.index)}-private" }, var.private_subnet_tags, var.tags)
}
...
resource "aws_eip" "nat" {
  ...
  tags = merge({ "Name" = "${var.environment}-${element(var.zones, count.index)}-eip" }, var.tags)
}
...
resource "aws_nat_gateway" "nat_gw" {
  ...
  tags = merge({ "Name" = "${var.environment}-${element(var.zones, count.index)}-nat" }, var.tags)
